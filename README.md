...
- **双语双栏编辑**：点击中文段落编辑（LaTeX 源码级），`Ctrl+Enter` 保存，英文自动翻译并写回 `.tex`；点击英文段落同样可编辑（保存后写回并回译中文）
- **章节对话**：每个章节底部的对话框针对**当前章节**提出修改要求（如「把实验描述改得更严谨」），LLM 直接修订英文、中文反向同步；LaTeX 保护校验保证公式/引用/命令不被破坏，违规修改会被拦截
- **公式校对（实验性）**：章节底部独立的公式对话框——
  - **数值代入**：为本章每个公式解释含义，并构造具体数值逐步演算
  - **向量/矩阵示例**：对向量/矩阵公式自行构造 2 维例子并完整计算（无需你填值）
  - **一键可视化**：为公式生成含义示意图（SVG）
  - **符号一致性（全篇）**：扫描全篇找出含义相同但符号不统一的公式（含裸宏如 `\mathcal{B}` 与 `B` 的冲突），给出统一记号建议
  - **专项提问**：只针对公式提问；LLM 给出的修正建议可逐条「应用」，做精确替换写回 `.tex` 并回译中文
- **保护校验**：翻译与修改全程校验数学式、`\citep`/`\ref` 等键、LaTeX 命令，丢失即警告
- **git 同步**：一键拉取（`pull --ff-only`）/ 提交推送；英文被外部修改的段落自动标记「中文过时」
- **多模型后端**：DeepSeek 默认，设置页可「新增对话模型」自定义任意 OpenAI 兼容后端
...
├── formula.py        # 公式校对（数值代入/向量矩阵示例/可视化/符号一致性）
